...
public class ClientApp {
    ...
    private static void updateBooks(Client client) {
        ...
        if (BooksIDString != null && !BooksIDString.trim().isEmpty()) {
            try {
                ...
                System.out.println("Ваш запрос неверен! BooksId является целым числом.");
            }
        }
    ...
    private static void deleteBooks(Client client) {
        ...
        try {
            ...
            System.out.println("Неверное значение booksId! Введите только одно целое число..");
        }
    }
    ...
    private static void createBooks(Client client) {
        ...
        if ((title != null && !title.trim().isEmpty())  &&
            ...
            try {
                Integer.parseInt(yearString.trim());
